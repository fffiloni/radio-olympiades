...
var Wave = (function () {
  ...
  function fromFile(file) {
    ...
    var getWave = undefined;
    var fdi = 0;
    var self = this;

    audio.addEventListener('loadedmetadata', function callee$2$0() {
  ...
  var drawWave = function drawWave(functionContext) {
    ...
    var options = functionContext.options;
    var ctx = functionContext.ctx;
    var h = functionContext.h;
    var w = functionContext.w;
    var Helper = functionContext.Helper;
    ...
    (function (window) {
      ...
      function smartCoordinates(args) {
        ...
        if (axis.x.indexOf(x) !== -1) {
          if (x === 'right') x = Math.floor(elmWidth - width);else if (x === 'center') if (radius) x = Math.floor(elmWidth / 2);else x = Math.floor(elmWidth / 2 - width / 2);else if (x === 'left') x = radius;
        } else if ((x + '').substr(-1) === '%') {
        ...
        if (axis.y.indexOf(y) !== -1) {
          if (y === 'top') y = radius;else if (y === 'center') if (radius) y = Math.floor(elmHeight / 2);else y = Math.floor(elmHeight / 2 - height / 2);else if (y === 'bottom') y = Math.floor(elmHeight - height);
        } else if ((y + '').substr(-1) === '%') {
      ...
      Origami.rotate = function (degrees) {
        ...
        if (typeof degrees === 'string') {
          // Slow
          if (degrees === 'slow') degrees = 2 * Math.PI / 60 * new Date().getSeconds() + 2 * Math.PI / 60000 * new Date().getMilliseconds();

          // Normal
          else if (degrees === 'normal') degrees = 2 * Math.PI / 30 * new Date().getSeconds() + 2 * Math.PI / 30000 * new Date().getMilliseconds();
  ...
  var drawRoundLayers = function drawRoundLayers(functionContext) {
    ...
    var w = functionContext.w;
    var Helper = functionContext.Helper;
    var canvasId = functionContext.canvasId;
  ...
  function visualize(data, canvasId, options, frame) {
    ...
    var functionContext = {
      data: data, options: options, ctx: ctx, h: h, w: w, Helper: this.Helper, canvasId: canvasId
    };
  ...
  Helper.prototype = {
    ...
    __rotatePoint__: function __rotatePoint__(_ref, _ref3, degree) {
      ...
      var originX = _ref32[0];
      var originY = _ref32[1];
    ...
    getPoints: function getPoints(shape, size, _ref4, pointCount, endPoints) {
      var _ref42 = _slicedToArray(_ref4, 2);

      var originX = _ref42[0];
      var originY = _ref42[1];
      var options = arguments.length <= 5 || arguments[5] === undefined ? {} : arguments[5];
      var _options$offset = options.offset;
      var offset = _options$offset === undefined ? 0 : _options$offset;
      var _options$rotate = options.rotate;
      var rotate = _options$rotate === undefined ? 0 : _options$rotate;
      var _options$customOrigin = options.customOrigin;
      var customOrigin = _options$customOrigin === undefined ? [] : _options$customOrigin;
      ...
      if (shape === "line") {
        ...
        for (var i = 0; i <= pointCount; i++) {
          ...
          var startingPoint = this.__rotatePoint__([originX + i * increment, originY - pointOffset], [originX, originY], degree);
          rtn.start.push(startingPoint);

          var endingPoint = this.__rotatePoint__([originX + i * increment, originY + endPoints[i] - pointOffset], [originX, originY], degree);
    ...
    drawCircle: function drawCircle(_ref5, diameter) {
      ...
      var lineColor = _options$lineColor === undefined ? this.ctx.strokeStyle : _options$lineColor;
    ...
    drawOval: function drawOval(_ref6, height, width) {
      ...
      var rotation = _options$rotation === undefined ? 0 : _options$rotation;
      var color = options.color;
      var _options$lineColor2 = options.lineColor;
      var lineColor = _options$lineColor2 === undefined ? this.ctx.strokeStyle : _options$lineColor2;
    ...
    drawSquare: function drawSquare(_ref7, diameter) {
    ...
    drawRectangle: function drawRectangle(_ref8, height, width) {
      ...
      var options = arguments.length <= 3 || arguments[3] === undefined ? {} : arguments[3];
      var color = options.color;
      var _options$lineColor3 = options.lineColor;
      var lineColor = _options$lineColor3 === undefined ? this.ctx.strokeStyle : _options$lineColor3;
      var _options$radius = options.radius;
      var radius = _options$radius === undefined ? 0 : _options$radius;
      var _options$rotate2 = options.rotate;
      var rotate = _options$rotate2 === undefined ? 0 : _options$rotate2;
    ...
    drawLine: function drawLine(_ref9, _ref10) {
      ...
      var lineColor = _options$lineColor4 === undefined ? this.ctx.strokeStyle : _options$lineColor4;
    ...
    drawPolygon: function drawPolygon(points) {
      var options = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];
      var color = options.color;
      var _options$lineColor5 = options.lineColor;
      var lineColor = _options$lineColor5 === undefined ? this.ctx.strokeStyle : _options$lineColor5;
      var _options$radius2 = options.radius;
      var radius = _options$radius2 === undefined ? 0 : _options$radius2;
      var _options$close = options.close;
      var close = _options$close === undefined ? false : _options$close;
